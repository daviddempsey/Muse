...
exports.populateLeaderboard = async function (
  ...
  for (let doc of docs) {
    ...
    getLocation(fsdb, otherUserId)
      .then((otherUserLocation) => {
        ...
        if (doc.id !== currUserId && distance <= distanceLimit) {
          doc
            .data()
            .in_harmony.then((otherUserEmail) => {
              computeCompatibility(admin, fsdb, currUserId, otherUserEmail)
                .then((compatability) => {
                  response.push(compatability);
                  return response;
                })
...
exports.findTopSimilarGenres = function (list1, list2) {
  ...
  // Sorts list based on difference then name
  similarities.sort((a, b) =>
    a.frequency_sum < b.frequency_sum
      ? 1
      : a.frequency_sum === b.frequency_sum
      ? a.name < b.name
        ? 1
  ...
  for (let i in similarities) {
    similarities[i]["score"] =
      similarities[i]["frequency_sum"] / totalGenreFrequency;
  }
